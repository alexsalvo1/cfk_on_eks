...
if [[ -e ./resources/sslcerts/client/truststore.jks ]];then
  rm ./resources/sslcerts/client/truststore.jks
else
  mkdir -p resources/sslcerts/client/
fi
...
for file in /tmp/trustCAs/*; do
  ...
  keytool -import \
        ...
        -alias "${fileName}" \
        -file "${file}" \
	-keystore ./resources/sslcerts/client/truststore.jks \
	-deststorepass "${jksPassword}" \
  -deststoretype pkcs12 \
	-noprompt
done
echo "Validate truststore"
keytool -list -v -keystore ./resources/sslcerts/client/truststore.jks -storepass "${jksPassword}"
